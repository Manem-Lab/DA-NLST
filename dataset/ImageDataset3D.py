...
from data_augmentation.RandAugment3D import RandAugment3D
...
class ImageDataset3D(Dataset, Randomizable):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.da = da
        if self.da == 'random':
            self.augment = RandAugment3D(n=2, magnitude=9)
    ...
    def __getitem__(self, index: int):
        ...
        if self.is_train:
            if self.da == 'no_aug':
                ...
                new_transforms_list = self.augment(new_transforms_list)
        if self.is_rgb:
